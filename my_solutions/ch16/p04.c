...
int main(void)
{
    ...
    for (;;) {
        ...
        switch (code) {
            ...
            case 'c': change_price(inventory, num_parts); break;
            case 'q': return 0;
...
/**********************************************************
 * change_price: prompts the user to enter a part number and
 * a price and replaces current price with a new one.
 **********************************************************/
void change_price(struct part inventory[], int num_parts)
{
    int i, number, change;
    ...
    if (i >= 0) {
        printf("Enter change in price on hand: ");
        scanf("%d", &change);
...
void insert(struct part inventory[], int *num_parts)
{
    ...
    /* Find position where new part should be inserted */
    int i = 0;
    while (i < *num_parts && inventory[i].number < part_number)
        i++;

    /* Shift parts to make room for the new one */
 ...
 * prints the name and quantity on hand; if not, prints an
 * error message.
 **********************************************************/
void search(struct part inventory[], int num_parts)
{
 ...
 * Parts are printed in ascending order.
 **********************************************************/
void print(struct part inventory[], int num_parts)
{
    int i;
