...
bool play_game(void){
    ...
    if (dice_sum == 7 || dice_sum == 11) {
        ...
        // Play again but winning condition is changed.
        while((dice_sum = roll_dice()) != point && dice_sum != 7) {
            printf("You rolled: %d\n", dice_sum);
            if (dice_sum == point){
                return true;
            } else if (dice_sum == 7){
...
int roll_dice(void){
    ...
    return rand() % 12; // no need to sum. Sum of 2 random numbers is a random number
}